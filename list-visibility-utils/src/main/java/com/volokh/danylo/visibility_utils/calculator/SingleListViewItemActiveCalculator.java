...
public class SingleListViewItemActiveCalculator extends BaseItemsVisibilityCalculator {
     ...
     *此方法计算下一个项目的可见性。
     * *有些情况下，下一个项目不能填写数据：*
     * * 1。当前数据是列表中的最后一项时。在这种情况下没有下一个数据*
     * 2。指数当前视图无法计算因为视图已经回收* @param itemspositiongetter
     * * @param currentiem -项目，主动权现在*
     * @param参数outnextitemdata出。它将填充下一个项目数据，如果一个存在
     */
    ...
    /**
     * 此方法计算前一项的可见性,*有一些情况下，以前的项目不能填写数据：
     * 1。当前数据是列表中的第一项时。在这种情况下，没有以前的数据*
     * 2。指数当前视图无法计算因为视图已经回收
     * @param itemsPositionGetter *
     * @param currentIem -项目，主动权现在*
     * @param参数outpreviousitemdata出。如果存在，它将填充以前的项目数据
     */
    ...
    public void onScrollStateIdle(ItemsPositionGetter itemsPositionGetter, int firstVisiblePosition, int lastVisiblePosition) {
        ...
        //因为Adapter添加了脚步View,在此做判断是都到达最后一个条目（脚步）的时候，就结束下面的方法
        if(lastVisiblePosition>=mListItems.size()){
            return;
        }
    ...
    protected void onStateFling(ItemsPositionGetter itemsPositionGetter) {
        ...
        Log.d(TAG, "onStateFling: mCurrentItem="+mCurrentItem);
