...
public class VideoItem implements ListItem, MultiItemEntity {
    ...
    public VideoItem(ItemCallback callback, VideoOnItemClickListener onItemClickListener, TopicClickListener topicClickListener, Activity context, int sceneType) {
     ...
     * @param position
     */
    public void onBindViewHolder(VideoListViewHolder holder, FollowVideoList.DataBean.ListsBean videoListInfo, int position, ScaleAnimation  followScaleAnimation) {
        this.helper=holder;
    ...
    private void bindData() {
        ...
        helper.setImageResource(R.id.iv_item_follow_icon,0==item.getIs_interest()?R.drawable.iv_follow_selector_gray:R.drawable.iv_icon_follow_true);//设置是否已点赞
        ...
        helper.video_player.setUp(proxyUrl, XinQuVideoPlayer.SCREEN_WINDOW_LIST, ConfigSet.getInstance().isPalyerLoop(),item.getDesp());
    ...
    private void onFollowUser(final VideoListViewHolder helper, FollowVideoList.DataBean.ListsBean data) {
     ...
     * @param data
     */
    private void onPrice(final VideoListViewHolder helper, FollowVideoList.DataBean.ListsBean data) {
                        ...
                        if(!context.isFinishing()){
                            if(jsonObject.length()>0&&1==jsonObject.getInt("code")){
                                ...
                                if(TextUtils.equals(Constant.PRICE_SUCCESS,jsonObject.getString("msg"))){
                                    ...
                                    helper.setImageResource(R.id.iv_item_follow_icon,R.drawable.iv_follow_selector_gray);
                                    helper.re_video_group.setIsPrice(false);
                                }
                                helper.setText(R.id.tv_item_follow_count,comentCount);
                                //首页,只需要处理个人中心的我的作品和我的收藏
