...
import com.video.newqu.ui.fragment.MediaLocationVideoListFragment;
...
public class MediaLocationVideoListActivity extends BaseActivity<ActivityMediaVideoListBinding> {
    ...
    private static final String TAG = MediaLocationVideoListActivity.class.getSimpleName();
    ...
    private MediaLocationVideoListFragment mMediaLocationVideoListFragment;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mMediaLocationVideoListFragment = new MediaLocationVideoListFragment();
        addReplaceFragment(mMediaLocationVideoListFragment,"相册");
        ...
        protected void onPostExecute(List<WeiXinVideo> weiXinVideos) {
            ...
            if(null!=mScanVideoProgress&&mScanVideoProgress.isShowing()&&!MediaLocationVideoListActivity.this.isFinishing()){
                if(null!=weiXinVideos&&weiXinVideos.size()>0){
                    ...
                    //让子界面刷新数据
                    if(null!= mMediaLocationVideoListFragment){
                        mMediaLocationVideoListFragment.updataAdapter(weiXinVideos);
                    }
                }else{
                    mScanVideoProgress.dismiss();
            ...
            if(10010==msg.what){
                ...
                if(null!=mMoivesListAdapter){
                    if(null!=videoInfo&&videoInfo.size()>0){
                        Logger.d(TAG,"videoInfo.size()="+videoInfo.size());
                    }
                    if(null!=videoInfo){
