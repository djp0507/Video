...
public class MediaRecordActivity extends AppCompatActivity implements ActivityCompat.OnRequestPermissionsResultCallback {
    ...
    private static final int MESSAGE_COMPLETED = 100;//视频合并完成
    private static String TAG = MediaRecordActivity.class.getSimpleName();
    private final static int PERMISSION_REQUEST_CAMERA_AUDIOREC = 1;   //摄像头、麦克风请求授权的请求码
    public static final int MAX_DURATION = 15 * 1000;  //最长拍摄时长 15s
    public static final int MIN_DURATION = 5 * 1000;  //最短拍摄时长 3s
                ...
                switch (view.getId()) {
                    ...
                    case R.id.btn_record_music:
                        ...
                        if(null!=mKSYRecordKit&&mKSYRecordKit.getRecordedFilesCount()<=0){
                            ...
                            ToastUtils.shoCenterToast("录制途中不能更换音乐，要更换音乐请先回删所有录制片段");
                        }
                        break;
                    //美颜
...
//        updateSpeedVolume();
    ...
    private void updateSpeedVolume() {
        if (mKSYRecordKit.getRecordSpeed() != 1.0f ) {
            ...
            ToastUtils.shoCenterToast("选择变速录制自动禁用了声音录制!");
        } else {
    ...
    public void onDestroy() {
        ...
        if(null!=mEffectFilters)mEffectFilters.clear();
        if(null!=mBeautyFilters)mBeautyFilters.clear();

        mTimeCountDownText=null;
        mMediaRecordFilterAdapter=null;mBtn_record_music=null;SOUND_CHANGE_TYPE=null;REVERB_TYPE=null;mCameraPreviewView=null;mCameraHintView=null;
        ...
        mRecord_filter_layout=null;mMediaMVAdapter=null;mEffectFilters=null;mBeautyFilters=null;mImgBeautyProFilter=null;mMediaEditBeautyAdapter=null;
        mMainHandler=null;mBtn_record_delayed=null;mBtn_flash=null;mBtn_camera_fail=null;mMediaEditSoundFilter=null;mMediaEditSoundReverAdapter=null;
...
//        mMaterialIndex = -1;
    ...
    private void startRecord() {
        ...
        //直接拍摄
            ...
            if(!b){
                ToastUtils.shoCenterToast("录制失败,请检查软件照相机使用权限或存储器!");
            }
        }
    }
        ...
        public void onError(int what, int msg1, int msg2) {
            ...
            switch (what) {
                ...
                case StreamerConstants.KSY_STREAMER_FILE_PUBLISHER_WRITE_FAILED:
                    ToastUtils.shoCenterToast("录制错误!");
                    stopRecord(false);
        ...
        switch (requestCode) {
            case PERMISSION_REQUEST_CAMERA_AUDIOREC: {
                ...
                if (grantResults.length > 0
                    ...
                    ToastUtils.shoCenterToast("要正常使用拍摄功能，请务必授予拍摄、录音权限！");
                }
                break;
            }
        }
