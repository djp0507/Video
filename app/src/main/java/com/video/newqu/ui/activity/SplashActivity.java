...
public class SplashActivity extends AppCompatActivity{
    ...
    private void goToNextActivity() {
        ...
        if(VideoApplication.mToday ==SharedPreferencesUtil.getInstance().getInt(Constant.SETTING_SPLASH, 0)){
            ...
            Logger.d(TAG,"第一次启动");
            SharedPreferencesUtil.getInstance().putInt(Constant.GRADE_LOGIN_COUNT,(SharedPreferencesUtil.getInstance().getInt(Constant.GRADE_LOGIN_COUNT, 0)+1));//连续统计登录天数
            SharedPreferencesUtil.getInstance().putInt(Constant.GRADE_PLAYER_VIDEO_COUNT,0);
            SharedPreferencesUtil.getInstance().putBoolean(Constant.GRADE_TODAY_TIPS,false);//还原当天不再显示评分弹窗
