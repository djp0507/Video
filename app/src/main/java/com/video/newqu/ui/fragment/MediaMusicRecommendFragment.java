...
import com.video.newqu.ui.presenter.MediaMusicRecommendPresenter;
...
public class MediaMusicRecommendFragment extends BaseMineFragment<FragmentMediaMusicHomeBinding> implements BaseQuickAdapter.RequestLoadMoreListener, MediaMusicRecommendContract.View,OnMediaMusicClickListener{

    private static final String TAG = MediaMusicRecommendFragment.class.getSimpleName();
    ...
    private MediaMusicRecommendPresenter mMediaMusicRecommendPresenter;
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        mMediaMusicInfoList= (List<MediaMusicCategoryList.DataBean>)  ApplicationManager.getInstance().getCacheExample().getAsObject(Constant.CACHE_MEDIA_RECORED_RECOMMEND_MUSIC);
        if(null==mMediaMusicInfoList) mMediaMusicInfoList=new ArrayList<>();
        mMediaMusicRecommendPresenter = new MediaMusicRecommendPresenter(getActivity());
        mMediaMusicRecommendPresenter.attachView(this);
        initAdapter();
...
//        });
    ...
    private void initAdapter() {
        ...
        emptyViewbindView.tvItemName.setText("音乐数据为空");
    ...
    private void loadMusicData() {
        if(null!=mMediaMusicRecommendPresenter&&!mMediaMusicRecommendPresenter.isHomeLoading()){
            page++;
            mMediaMusicRecommendPresenter.getRecommendMusicList(null,page,pageSize);
        }else{
//            bindingView.swiperefreshLayout.setRefreshing(false);
    ...
    @Override
    public void showRecommendMusicList(List<MediaMusicCategoryList.DataBean> data) {
...
//            bindingView.swiperefreshLayout.setRefreshing(false);
            ...
            if(1==page){//替换最新缓存
                ...
                ApplicationManager.getInstance().getCacheExample().remove(Constant.CACHE_MEDIA_RECORED_RECOMMEND_MUSIC);
                ApplicationManager.getInstance().getCacheExample().put(Constant.CACHE_MEDIA_RECORED_RECOMMEND_MUSIC, (Serializable) mMediaMusicInfoList, Constant.CACHE_TIME);
    ...
    @Override
    public void showRecommendMusicEmpty(String data) {
        if(null!= mMediaMusicRecommendAdapter &&null!=getActivity()&&!getActivity().isFinishing()){
//            bindingView.swiperefreshLayout.setRefreshing(false);
            showContentView();
    ...
    @Override
    public void showRecommendMusicError(String data) {
        if(null!= mMediaMusicRecommendAdapter &&null!=getActivity()&&!getActivity().isFinishing()){
//            bindingView.swiperefreshLayout.setRefreshing(false);
                        ...
                        if(null!=music_id){
                            if(null!=mMediaMusicRecommendAdapter){
                                ...
                                if(null!=musicList&&musicList.size()>0){
                                    ...
                                    //替换最新缓存
                                    ApplicationManager.getInstance().getCacheExample().remove(Constant.CACHE_MEDIA_RECORED_RECOMMEND_MUSIC);
                                    ApplicationManager.getInstance().getCacheExample().put(Constant.CACHE_MEDIA_RECORED_RECOMMEND_MUSIC, (Serializable) musicList, Constant.CACHE_TIME);
                                }
                            }
                        }
                    }else{
    ...
    public void onMessageEvent(MessageEvent event) {
        ...
        //只处理与自己相关的事件
        if(null!=event&&TextUtils.equals(Constant.EVENT_UPDATA_MUSIC_PLAYER,event.getMessage())&&0==event.getType()){
            WindowVideoPlayer.releaseAllVideos();
            if(null!=mMediaMusicRecommendAdapter){
    ...
    public void onDestroy() {
        if(null!=mMediaMusicRecommendPresenter){
            mMediaMusicRecommendPresenter.detachView();
        }
        super.onDestroy();
    }
    ...
    public void onLikeClick(MediaMusicCategoryList.DataBean data) {
        if(!getActivity().isFinishing()&&null!=mMediaMusicRecommendPresenter&&!mMediaMusicRecommendPresenter.isLikeIng()){
            showProgressDialog("操作中...",true);
            mMediaMusicRecommendPresenter.likeMusic(data.getId(),data.getIs_collect());
        }
    }
