...
public class SettingFragment extends BaseFragment<FragmentSettingBinding> {
    ...
    protected void initViews() {
        ...
        //设置没尅有在XML文件中指定高度的View的高度

        //先测量目标组件的宽高
        int width =View.MeasureSpec.makeMeasureSpec(0,View.MeasureSpec.UNSPECIFIED);
        int height =View.MeasureSpec.makeMeasureSpec(0,View.MeasureSpec.UNSPECIFIED);
        bindingView.reWatermark.measure(width,height);
        ...
        bindingView.tvVerstionCode.setText("当前版本 "+ Utils.getVersion(getActivity()));
        //回显状态
    ...
    private void loginOut() {
        ...
        TagAliasOperatorHelper.getInstance().setOnAliasChangeListener(new TagAliasOperatorHelper.OnAliasChangeListener() {
            ...
            public void onChange(JPushMessage jPushMessage) {
                Log.d("设置", "onChange: 收到别名变化"+jPushMessage.toString());
            }
        });
        ...
        Log.d("注销", "loginOut: loginType="+loginType);
        SHARE_MEDIA platformInfo=null;
        switch (loginType){
            //QQ
            case 1:
                Logger.d("SettingsFragment","QQ登录用户");
                Log.d("SettingsFragment","");
                platformInfo= SHARE_MEDIA.QQ;
                break;
            //微信
            case 2:
                Logger.d("SettingsFragment","微信登录用户");
                platformInfo= SHARE_MEDIA.WEIXIN;
                break;
            //微博用户
            case 3:
                Logger.d("SettingsFragment","微博登录用户");
                platformInfo= SHARE_MEDIA.SINA;
                break;
            //手机号用户
            case 4:
                Logger.d("SettingsFragment","手机登录用户");
                platformInfo=null;
                break;
        }
    ...
    private UMAuthListener deleteAuthListener=new UMAuthListener() {
        ...
        public void onComplete(SHARE_MEDIA share_media, int i, Map<String, String> map) {
            Logger.d("SettingsFragment","onComplete:已注销");
            deleteUserData();
        }
