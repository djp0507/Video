...
public class LoginGroupActivity extends BaseActivity<ActivityLoginGroupBinding> implements LoginContract.View {
    ...
    private static final String TAG = LoginGroupActivity.class.getSimpleName();
    ...
    public void closeForResult() {
        if(null!=VideoApplication.getInstance().getUserData()&&!TextUtils.isEmpty(VideoApplication.getInstance().getUserData().getId())){
            ...
            ToastUtils.shoCenterToast("登录异常，请重新登录!");
        }
    }
    ...
    UMAuthListener LoginAuthListener = new UMAuthListener() {
            ...
            try{
                if(null!=data&&data.size()>0){
                    ...
                    if(platform== SHARE_MEDIA.SINA){
                        Logger.d("LoginActivity","onComplete:新浪微博用户登录,platform="+platform);
                    ...
                    //微信、QQ
                        ...
                        Logger.d("LoginActivity", "onComplete: 微信、QQ用户登录，platform="+platform);
                    ...
                    if(TextUtils.isEmpty(userDataInfo.getNickname())&&TextUtils.isEmpty(userDataInfo.getFigureurl_qq_2())){
                        Logger.d("LoginActivity","onComplete: 授权成功");
                        login(platform);
                    }else{
                        //登录App成功
                        Logger.d("LoginActivity","onComplete: 授权登陆成功");
                        //防止微博
                    ...
                    closeProgressDialog();
                    ToastUtils.shoCenterToast("登录失败，请重试!");
                }
            }catch (Exception e){
                closeProgressDialog();
                ToastUtils.shoCenterToast("登录失败，请重试!");
        ...
        public void onCancel(SHARE_MEDIA platform, int action) {
            closeProgressDialog();
            ToastUtils.shoCenterToast("登录取消");
        }
    };
    ...
    public void onBackPressed() {
        Logger.d(TAG,"onBackPressed--回退栈数量"+getSupportFragmentManager().getBackStackEntryCount());
        //只剩登录一个界面了
