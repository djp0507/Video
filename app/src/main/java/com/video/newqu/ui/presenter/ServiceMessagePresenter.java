...
public class ServiceMessagePresenter extends RxPresenter<ServiceMessageContract.View> implements ServiceMessageContract.Presenter<ServiceMessageContract.View> {
    ...
    private static final String TAG = ServiceMessagePresenter.class.getSimpleName();
        ...
        protected void onPostExecute(List<File> files) {
            ...
            builder.build().execute(new StringCallback() {
                ...
                public void onError(Call call, Exception e, int id) {
                    mView.showSendMessageError(e.getMessage());
                }

                @Override
                public void inProgress(float progress, long total, int id) {
                    super.inProgress(progress, total, id);
                    mView.onUpdataProgress(progress);
                }
                        ...
                        try {
                            ...
                            if(jsonObject.length()>0){
                                if(1==jsonObject.getInt("code")){
                                    mView.showSendMessageResult(jsonObject.getString("msg"));
                                }else{
                                    mView.showSendMessageError(jsonObject.getString("msg"));
                                }
                            }
