...
public class MediaScreenShortActivity extends BaseActivity<ActivityMediaScreenshortBinding> implements View.OnClickListener {
    ...
    private void startCoverSeek() {
        ...
        bindingView.coverSeekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                ...
                Logger.d("截图","mSeekTime="+mSeekTime);
            }
                ...
                case R.id.iv_seek_reset:
                    switch (event.getAction()) {
                        case MotionEvent.ACTION_UP:
                            Logger.d("OnTouchListener","iv_unseek_reset--ACTION_UP");
                        ...
                        case MotionEvent.ACTION_MOVE:
                            Logger.d("OnTouchListener","iv_seek_reset--ACTION_MOVE");
                            break;
                        case MotionEvent.ACTION_DOWN:
                            seetCursor=true;

                            sHandler.postDelayed(NextSeekToRunnable,mAutoDurtion);
                            initSeekThread();
                            mStopSeekThumbnail = false;

                            seekShortToReset(true);
                            seekLongToReset(true);
                            Logger.d("OnTouchListener","iv_seek_reset--ACTION_DOWN");
                            break;
                    }
                    return true;
                //减
                case R.id.iv_unseek_reset:
                    ...
                    switch (event.getAction()) {
                        case MotionEvent.ACTION_UP:
                            ...
                            break;
                        case MotionEvent.ACTION_MOVE:
                            Logger.d("OnTouchListener","iv_unseek_reset--ACTION_MOVE");
                            break;
                        case MotionEvent.ACTION_DOWN:
                            seetCursor=false;
                            ...
                            seekLongToReset(true);
                            Logger.d("OnTouchListener","iv_unseek_reset--ACTION_DOWN");
                            break;
                    }
                    return true;
            }
    ...
    private void seekLongToReset(boolean seek) {
        if(seek){
            sHandler.postDelayed(NextSeekToRunnable,0);
        }else{
            sHandler.removeCallbacks(NextSeekToRunnable);
    ...
    private Runnable NextSeekToRunnable = new Runnable() {
        ...
        public void run() {
            ...
            Logger.d(TAG,"progress=Long,start"+progress);
            //加，每一秒进度条向上加5秒
            if(seetCursor){
                if(progress>=100){
                    progress=100;
            ...
            Logger.d(TAG,"progress=Long,end"+progress);
    ...
    private void seekShortToReset(boolean seek) {
        int progress = bindingView.coverSeekBar.getProgress();
        Logger.d(TAG,"progress=start"+progress);
        //加
        if(seek){
            if(progress>=100){
                progress=100;
        ...
        Logger.d(TAG,"progress=end"+progress);
        bindingView.coverSeekBar.setProgress(progress);
    }
