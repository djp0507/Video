...
public class VerticalPlayeViewPager extends BasePager<VerticalPagerVideoPlayLayoutBinding>implements TopicClickListener, VideoDetailsContract.View, ShareFinlishListener {
    ...
    public void initData() {
        ...
        if(null!=mContext&&!mContext.isFinishing()&&null!=bindingView){
            ...
            setVideoRatio(Integer.parseInt(mVideoBean.getType()),bindingView.videoPlayer);
//            //布局的全局宽高变化监听器
...
//            });
            ...
            bindingView.videoPlayer.setOnPlayerCallBackListener(new WindowVideoPlayer.OnPlayerCallBackListener() {
                ...
                public void startPlayCallBack() {
                    //上传播放记录
                    if(null!=mVideoBean) ApplicationManager.getInstance().postVideoPlayState(mVideoBean.getVideo_id(),(int)bindingView.videoPlayer.getCurrentPositionWhenPlaying(), 1, null);
...
//        danmaku.borderColor = CommonUtils.getColor(R.color.translucent_65); //边框的颜色
    ...
    private void saveLocationHistoryList() {
        ...
        new Thread(){
            ...
            public void run() {
                ...
                try {
                    if(null!=bindingView){
                        PostPlayStateHanderUtils.postVideoPlayState(mVideoBean.getVideo_id(),(int)bindingView.videoPlayer.getCurrentPositionWhenPlaying(),0,null);
                    }
                }catch (Exception e){
    ...
    private void showMenu() {
        ...
        if(null!=mContext&&!mContext.isFinishing()){
            ...
            //是发布此视频的用作者自己
            if(null!=VideoApplication.getInstance().getUserData()&&TextUtils.equals(mVideoBean.getUser_id(),VideoApplication.getLoginUserID())){
                //原本私密的视频
                    ...
                    switch (itemID) {
                        ...
                        case 6:
                            ...
                            if(!showAutoComment){
                                ...
                                Logger.d(TAG,"bindingView.videoPlayer.isPlaying()="+bindingView.videoPlayer.isPlaying());
    ...
    public void showPriseResult(String data) {
        ...
        if(TextUtils.isEmpty(data)){
            ...
            ToastUtils.shoCenterToast("点赞失败");
        }else{
            try {
