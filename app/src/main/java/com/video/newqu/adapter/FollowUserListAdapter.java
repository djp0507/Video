...
public class FollowUserListAdapter extends BaseQuickAdapter<FollowUserList.DataBean.ListBean,BaseViewHolder> {
    ...
    private static final String TAG = FollowUserListAdapter.class.getSimpleName();
        ...
        try {
            if(null!=item){
                ...
                if(1==objectType&&1==item.getIs_follow()){
                    helper.setVisible(R.id.iv_item_menu,true);
                    helper.setVisible(R.id.btn_re_follow,false);
                    //自己的关注列表和未关注
                }else if(1==objectType&&0==item.getIs_follow()){
                    helper.setVisible(R.id.iv_item_menu,false);
                    helper.setVisible(R.id.btn_re_follow,true);
                    //别人的关注列表
                }else{
                    helper.setVisible(R.id.iv_item_menu,false);
                    helper.setVisible(R.id.btn_re_follow,false);
...
//                helper.setVisible(R.id.view_line,helper.getPosition()==getData().size()-1?false:true);
                ...
                helper.setOnClickListener(R.id.btn_re_follow, new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        onFansClickListener.onFollowUser(helper.getPosition(),item);
                    }
                });
                //菜单
                helper.setOnClickListener(R.id.iv_item_menu, new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        onFansClickListener.onMenuClick(item);
                    }
                });
                //条目的点击事件
