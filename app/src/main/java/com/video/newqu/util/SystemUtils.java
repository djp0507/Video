...
import static com.video.newqu.util.DownloadApkTask.mInstance;
...
public class SystemUtils {
    ...
    public static int getNumCores() {
        ...
        try {
            ...
            Logger.d(TAG, "CPU Count: " + files.length);
            //Return the number of cores (virtual CPU devices)
            return files.length;
        } catch (Exception e) {
            //Print exception
            Logger.d(TAG, "CPU Count: Failed.");
            e.printStackTrace();
    ...
    public static String getTopActivity() {
        ...
        Logger.d("视频扫描", "当前显示的界面" + component.getClassName());
...
//                    return intToIp(ipAddress);
     ...
     * 获取经纬度
     */
    public static String[] getLocationID() throws Exception {
        try {
            //获取地理位置管理器
            ...
            if (null != providers && providers.size() > 0) {
                ...
                if (providers.contains(LocationManager.GPS_PROVIDER)) {
                    ...
                    Logger.d("MainPresenter", "run: 权限不足或没有可用的GPS");
                }
                if (null != locationProvider) {
                    ...
                    if (ActivityCompat.checkSelfPermission(VideoApplication.getInstance().getApplicationContext(), Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && ActivityCompat.checkSelfPermission(VideoApplication.getInstance().getApplicationContext(), Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                        ...
                        ToastUtils.shoCenterToast("getLocationID");
                        return null;
                    }
    ...
    public void startAppDetailsInfoActivity(Activity context, int requestCode) {
        ...
        try {
            ...
            ToastUtils.shoCenterToast("打开权限设置界面错误！请到应用管理界面手动开启权限！");
        }
    }
