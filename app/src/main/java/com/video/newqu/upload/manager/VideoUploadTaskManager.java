...
public class VideoUploadTaskManager {
                    ...
                    for (int i = 0; i < data.size(); i++) {
                        ...
                        if(null!=uploadVideoInfo){
                            if(null!=ossTaskMap&&ossTaskMap.size()>0){
                                ...
                                if(!isExit){
                                    Logger.d(TAG,"任务不存在，添加新上传任务--"+uploadVideoInfo.getId());
    ...
    private class UploadVideoAsyncTask {
        ...
        public void execute() {
            if(null!=mUploadInfo){
                //上传构造请求
                PauseableUploadRequest request = new PauseableUploadRequest(Constant.STS_BUCKET, "Video/"+mUploadInfo.getVideoFileKey(), mUploadInfo.getFilePath(), PART_SIZE);
                //上传进度监听
                ...
                try {
                    ...
                    if(TextUtils.isEmpty(mUploadInfo.getUploadID())){
                        ...
                        Logger.d(TAG,"新任务--uploadID"+uploadID);
                    }else{
                        uploadID=mUploadInfo.getUploadID();
                        Logger.d(TAG,"旧任务-=uploadID="+uploadID);
                    }
                    //添加进任务管理实例中
                    if(null!=ossTaskMap){
                    ...
                    Logger.d(TAG,"上传失败"+e.getMessage()+",错误码:"+e.getErrorCode());
                }
                catch (ClientException e) {
                    e.printStackTrace();
                    if(null!=ossTaskMap&&ossTaskMap.size()>0) ossTaskMap.remove(mUploadInfo.getId());//移除上传任务栈中的元素
                    ...
                    Logger.d(TAG,"上传失败"+e.getMessage()+",错误码:"+e.getCause());
                } catch (IOException e) {
                    e.printStackTrace();
                    if(null!=ossTaskMap&&ossTaskMap.size()>0) ossTaskMap.remove(mUploadInfo.getId());//移除上传任务栈中的元素
                    if(null!=mVideoUploadListener){
    ...
    private void setUserPrams(PauseableUploadTask pauseableUploadTask, UploadVideoInfo videoInfo) {
        ...
        Logger.d(TAG,"mParamVars="+mParamVars);
