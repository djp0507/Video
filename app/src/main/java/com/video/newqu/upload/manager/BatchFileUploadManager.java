...
public class BatchFileUploadManager {
    ...
    OSSCredentialProvider credetialProvider = new OSSFederationCredentialProvider() {
        ...
        public OSSFederationToken getFederationToken() {
            ...
            try {
                ...
                Logger.d(TAG, "getFederationToken: token="+token);
    ...
    public void upload(List<WeiChactVideoInfo> uploadVideoList){
        ...
        Logger.d(TAG,"upload上传任务个数"+uploadVideoList.size());
    ...
    private class UploadFileAsyncTask {
        ...
        public void execute(){
            ...
            serviceParams.put("object", "Video/" + durtionMD5);
                        ...
                        try {
                            ...
                            if(null!=jsonObject&&jsonObject.length()>0){
                                ...
                                if (0 == jsonObject.getInt("code")) {
                                    Logger.d(TAG,"服务器端不存在此文件"+videoInfo.getID());
                                    String durtionMD5 = StringUtils.stringToMd5(videoInfo.getVideo_durtion());
                                    //上传构造请求
                                    PauseableUploadRequest request = new PauseableUploadRequest(Constant.STS_BUCKET, "Video/"+durtionMD5, videoInfo.getFilePath(), PART_SIZE);
                                    //上传进度监听
                                    ...
                                    try {
                                        ...
                                        String fileMd5 = BinaryUtil.calculateMd5Str(videoInfo.getFilePath());
                                        String totalMd5 = BinaryUtil.calculateMd5Str((fileMd5 + Constant.STS_BUCKET + "Video/"+durtionMD5 + String.valueOf(PART_SIZE)).getBytes());
                                    ...
                                    Logger.d(TAG,"服务端已存在此文件"+videoInfo.getID());
        ...
        if(null!=ossTaskMap&&ossTaskMap.size()>0){
            ...
            while (iterator.hasNext()) {
                ...
                if(videoInfo.getId()==next.getKey()){
                    ...
                    AbortMultipartUploadRequest abort = new AbortMultipartUploadRequest(Constant.STS_BUCKET, "Video/"+objectKey, videoInfo.getUploadID());
    ...
    private void setUserPrams(PauseableUploadTask pauseableUploadTask, WeiChactVideoInfo videoInfo) {
        ...
        try {
            locationID = SystemUtils.getLocationID();
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        mParamVars.put("x:device_longitude",null==locationID?"0":locationID[0]);//经度
        mParamVars.put("x:device_latitude",null==locationID?"0":locationID[1]);//纬度
