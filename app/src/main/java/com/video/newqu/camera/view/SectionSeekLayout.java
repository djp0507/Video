package com.video.newqu.camera.view;
...
import com.video.newqu.camera.videorange.VideoThumbnailTask;
...
public class SectionSeekLayout extends RelativeLayout {
    ...
    private static  int ITEM_WIDTH = 105;  //单个缩略图的宽度
    private static final int INTERVAL = 5; // 用于监听惯性滑动的消息间隔，单位毫秒
    private static final int THUMBNAIL_INTERVAL = 2000; //获取缩略图的间隔
    ...
    private WeakReference<KSYEditKit> mKsyEditKitWeakReference;
    private WeakReference<Bitmap> mMDefaultBmpWeakReference;

    public void setThumbWidth(int thumbWidth) {
        ITEM_WIDTH=thumbWidth;
    }
    ...
    public void init(long duration, KSYEditKit kit) {
        ...
        mKsyEditKitWeakReference = new WeakReference<KSYEditKit>(kit);
        mRectLeftPosition = 0;
        mRectRightPosition = mScreenWidth;
        mSectionMap = new SparseArray<>();
        mMDefaultBmpWeakReference = new WeakReference<Bitmap>(BitmapFactory.decodeResource(mContext.getResources(), R.drawable.iv_video_errror));
        //每1s取一张图片，第一个Item和最后一个Item为空白，宽度为屏幕宽度一半，方便从屏幕中间开始预览
        mItemCount = (int) (duration / THUMBNAIL_INTERVAL) + 2;
        SimpleImageAdapter simpleAdapter = new SimpleImageAdapter();
        LinearLayoutManager layoutManager = new LinearLayoutManager(mContext);
        layoutManager.setOrientation(LinearLayoutManager.HORIZONTAL);
        mThumbView.setLayoutManager(layoutManager);
        mThumbView.setAdapter(simpleAdapter);
    ...
    public class SimpleImageAdapter extends RecyclerView.Adapter<SimpleImageAdapter.ImageHolder> {
        ...
        public void onBindViewHolder(ImageHolder holder, int position) {
            if (position == 0 || position == mItemCount - 1) {
                ...
                holder.image.setLayoutParams(params);
                if(null!=mMDefaultBmpWeakReference&&null!=mMDefaultBmpWeakReference.get()){
                    holder.image.setImageBitmap(mMDefaultBmpWeakReference.get());
                }
                ...
                if(null!=mKsyEditKitWeakReference&&null!=mKsyEditKitWeakReference.get()){
                    VideoThumbnailInfo info = new VideoThumbnailInfo();
                    info.mWidth = ITEM_WIDTH * 3;
                    VideoThumbnailTask.loadBitmap(mContext, holder.image, null, currentTime, info, mKsyEditKitWeakReference.get(), null);
                }
            }
        }
