...
public class VideoRangeSeekBar extends View {
    ...
    private void onActionDown(int pointerId, float x, float y) {
        ...
        if(null!=mRangeBarChangeListener){
            mRangeBarChangeListener.onEventDown(change);
        }
    }
    ...
    private void onActionUp(int pointerId, float x, float y) {
        ...
        if(null!=mRangeBarChangeListener){
            mRangeBarChangeListener.onEventUp(change);
        }
    }
    ...
    public  interface OnRangeBarChangeListener {
        ...
        int RIGHT_CHANGE = 2;
        int BOTH_CHANGE = 3;
        void onIndexChangeListener(VideoRangeSeekBar rangeBar, float rangeStart, float rangeEnd, int change, boolean toEnd);
        void onActionUp();
        void onEventDown(int mode);
        void onEventUp(int mode);
    }
