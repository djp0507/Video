package com.video.newqu.camera.videorange;
...
public class VideoThumbnailTask extends AsyncTask<Long, Void, Bitmap> {
    ...
    private static boolean isScreenThubm;//是否正在截图
    ...
    public VideoThumbnailTask(Context context, ImageView imageView, long ms, VideoThumbnailInfo videoThumbnailData, KSYEditKit retriever, View next, boolean h265file) {
        mContext = context;
    ...
    public static void loadBitmap(Context context, ImageView imageView, Bitmap defaultBitmap, long ms, VideoThumbnailInfo videoThumbnailData, KSYEditKit retriever, View next) {
        if (cancelPotentialWork(ms, imageView)) {
            ...
            final VideoThumbnailTask task = new VideoThumbnailTask(context, imageView, ms, videoThumbnailData, retriever, next, h265File);
            mWorkTaskNum++;
    ...
    static class AsyncDrawable extends BitmapDrawable {
        ...
        public AsyncDrawable(Resources res, Bitmap bitmap, VideoThumbnailTask bitmapWorkerTask) {
            super(res, bitmap);
            bitmapWorkerTaskReference = new WeakReference<VideoThumbnailTask>(bitmapWorkerTask);
        }
